...
# Climate model emulation with AI <br><span style="font-size: 10px;">Collaborators: James Duncan, Elynn Wu, Surya Dheeshjith, Adam Subel, Troy Arcomano, Spencer Clark, Brian Henn, Anna, Kwa, Jeremy McGibbon, Andre Perkins, Carlos Fernandez-Granda, Julius Busecke, Oli Watt-Meyer, Bill Hurlin, Alistair Adcroft, Laure Zanna, Chris Bretherton</span>
...
<span style="font-size: 16px;">Coupled climate models have been the cornerstone of Earth system research for decades and have allowed us to answer critical questions about how Earth's climate responds to changes in greenhouse gas emissions and therefore what the future might look like under different warming scenarios. However, these simulations are expensive. The state-of-the-art GFDL climate model CM4 has a throughput of 16 SYPD (Simulated Year Per Day) on 5535 CPU cores. To accurately understand climate variability and change we need to run long simulations---the deep ocean takes hundreds to thousands of years to reach an equilibrium, or *spun-up*, state. A 100 year simulation of CM4 takes almost 1 week and requires massive resources, meaning that we often do not run simulations long enough to appropriately spin up the ocean. Recent work in the space of AI and climate has shown that AI models can skillfully simulate individual Earth system components. For example, [ACE](https://doi.org/10.1038/s41612-025-01090-0) and [Samudra](https://doi.org/10.1029/2024GL114318) are auto-regressive AI emulators of the full 3D atmosphere and ocean, respectively. When forced with observed boundary conditions in the style of AMIP and OMIP experiments, these emulators can integrate, or *rollout*, stably for decades, while accurately capturing the mean state and variability of the atmosphere and ocean. In this latest pre-print, led by James Duncan at the Allen Institute for AI (Ai2), we couple the 3D atmosphere and ocean emulators, ACE and Samudra, to produce the first coupled climate emulator, SamudrACE. SamudrACE is a 1-degree resolution coupled model that has been trained on a coarsened 200-year CM4 pre-industrial control simulation. SamudrACE is significantly faster than CM4, achieving 1500 SYPD on 1 H100 GPU. This speed makes it a fast and flexible tool for modelling long-term climate and large-ensemble analysis for studying extreme events. For more information on SamudrACE, check out our [M$^2$LInES blog post](https://medium.com/@lz1955/samudrace-a-fast-accurate-efficient-3d-coupled-climate-ai-emulator-fcef3c60b079)! </span><br>

### FloeNet: global sea ice emulation
 <span style="line-height: 0;">[HuggingFace Repository](https://huggingface.co/M2LInES/FloeNet-OM4) | [Link to paper](https://doi.org/10.48550/arXiv.2603.12449)</span>

<img src="http://William-gregory.github.io/images/FloeNet_dynamics.gif" alt="arXiV2" width="300" align="right"/>
<span style="font-size: 16px;">Since SamudrACE, we have been working to improve the fidelity of coupled climate simulations with AI emulators. One major component of this work has involved improving high-latitude processes. Our latest publication introduces FloeNet, a mass-conserving global sea ice model, based entirely on AI. FloeNet is an auto-regressive graph neural network which has been trained to emulate the GFDL numerical sea ice model, SIS2. The animation on the right shows the wind stress forcing overlain on FloeNet's sea ice concentration. Notice how FloeNet responds rapidly to atmospheric dynamics, with storms breaking up the sea ice to produce areas of low ice concentration. In our paper, we evaluate 140-year rollouts under a pre-industrial control climate and a climate with increasing 1% per year atmospheric CO2 concentration. Under these conditions, we show how FloeNet generates accurate thermodynamic and dynamic responses to forcing. FloeNet is also over 3x faster than the original numerical model and requires significantly fewer resouces. A 140-year simulation with FloeNet takes 4.75 hours on 1 GPU, whereas SIS2 takes over 17 hours on 4671 CPUs.</span><br>
...
<div align="center"><img src="http://William-gregory.github.io/images/PA_forecasts.png" alt="TC1" width="75%"/></div>
